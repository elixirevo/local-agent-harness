...
| gemma4:12b (native) | 6/6 | 6/6 | **완전 천장**. 효율은 오히려 역전: find-change 20.3s→24.9s(+23%), 스텝 4.0→5.0 — 유능한 모델은 이미 최단 경로인데 워크플로우가 스텝을 추가함. test-fix는 도구 에러 2→0 |
| llama3.2 (text) | 1/6 | 1/6 | **리프트 없음** — 런당 도구 에러 ~20, 가드 다수. 병목이 계획이 아니라 **스텝 실행**(Edit 불일치 등)이라 계획을 줘도 못 따라감 |

**가설 기각**: "약한 모델일수록 스킬 리프트가 크다"는 이 과제 세트에서 성립하지 않았다. 모델 크기 축 전체(3B 텍스트 / e2b / 12B, 2026-07-11 12b 추가 실측)에서 성공률 리프트 0 — 게다가 12B에서는 스킬이 시간을 늘렸다. 약한 모델의 진짜 병목은 계획 수립이 아니라 개별 스텝 실행 신뢰성이었다. (참고: 베이스라인 프롬프트가 이미 꽤 지시적이었고, 과제가 2~4스텝의 범용 작업이라 스킬의 여지가 작았던 한계도 있다.)
