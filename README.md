...
힌트 메뉴가 열려 있으면 **↑↓ 또는 Tab으로 항목을 선택하고 Enter로 입력창에 채운다** — 한 번
더 Enter를 누르면 실행된다(인자가 필요한 명령은 채운 뒤 이어서 입력).
...
config `saveSessions: false`). 복원은 `harness -c`(마지막 세션 이어가기) 또는
`--resume <id|last>`. 저장된 세션이 있는 디렉터리에서 새로 시작하면 배너에 `-c` 팁이 뜬다.
