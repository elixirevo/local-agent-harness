...
**현재 상태: Phase 0** — 프로바이더 계층 + 스트리밍 REPL. 도구 호출 루프는 Phase 1에서 추가된다.
...
npm start -- -m qwen3:8b  # 모델 지정
npm start -- -p "안녕"     # one-shot 모드
...
- **Ollama**: `prompt_eval_count`는 0.30 기준 전체 프롬프트 토큰 수를 보고하므로(캐시 여부 무관, 실측 확인)
  prefill 시간으로 캐시 적중을 판단한다.
